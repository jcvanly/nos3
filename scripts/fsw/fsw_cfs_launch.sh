...
do
    ...
    gnome-terminal --tab --title=$SC_NUM" - Arch Sim"   -- $DFLAGS -v $SIM_DIR:$SIM_DIR --name $SC_NUM"-arch-sim"   -v /dev/shm:/dev/shm --network=$SC_NETNAME -w $SIM_BIN $DBOX ./nos3-single-simulator $SC_CFG_FILE arch-sim
